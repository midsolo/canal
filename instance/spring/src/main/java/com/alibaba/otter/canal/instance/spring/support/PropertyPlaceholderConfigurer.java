...
/**
 * 扩展Spring的{@linkplain org.springframework.beans.factory.config.PropertyPlaceholderConfigurer}，
 * 增加默认值的功能。例如：${placeholder:defaultValue}，假如placeholder的值不存在，则默认取得defaultValue。
 * <p>
 * base-instance.xml
 *   └── 加载顺序:
 *       → classpath:canal.properties (全局配置)
 *       → classpath:{destination}/instance.properties（实例配置）
 */
