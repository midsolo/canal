...
public interface CanalConnector {
    ...
    // 链接对应的canal server。发送的数据包PacketType类型为：HANDSHAKE，CLIENTAUTHENTICATION
    ...
    // 客户端订阅，不提交客户端filter，以服务端的filter为准。发送的数据包PacketType类型为：SUBSCRIPTION
    ...
    // 获取数据，自动进行确认。拿够batchSize条记录或者超过timeout时间；果timeout=0，则阻塞至拿到batchSize记录才返回
    ...
    // 进行batchId的确认。确认之后，小于等于此batchId的Message都会被确认
